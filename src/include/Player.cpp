...
inline namespace Object
{
                      ...
                      const vector<pair<Point, Point>> &lines)
    {
        ...
        for (int i = 0; i <= FOV; i += DTHETA)
        {
            ...
            const Point ray = Geometry::cross(Point(0, 0, 1), Point(x_v, y_v, 1));
            for (auto &line : lines)
            {
                auto [p1, p2] = line;
                ...
                {
                    int x2 = std::min(p1.x, p2.x),
                        y2 = std::min(p1.y, p2.y),
                        x3 = std::max(p1.x, p2.x),
                        y3 = std::max(p1.y, p2.y);
                    bool chosen = false;
                    if ((int)Q.x >= x2 && (int)Q.x <= x3 &&
                        (int)Q.y >= y2 && (int)Q.y <= y3)
                        chosen = 1;
                    if (chosen)
                    {
                        end_point = {(Q.x + x1 < EPSILON ? 0 : Q.x + x1), (y1 - Q.y < EPSILON ? 0 : y1 - Q.y)};
            ...
            // debug(end_point);
            end_points.push_back(Vertex(Vector2f(end_point.first, end_point.second)));
        }
