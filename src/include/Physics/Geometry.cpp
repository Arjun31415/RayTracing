...
Geometry::Point<T> Geometry::cross(Geometry::Point<T> a, Geometry::Point<T> b)
{
    ...
    return Geometry::Point<T>(y0 * z1 - z0 * y1 + 0.00, z0 * x1 - x0 * z1 + 0.00,
                              x0 * y1 - y0 * x1 + 0.00);
}
template <typename T>

int Geometry::intersection(Geometry::Point<T> p0, Geometry::Point<T> p1, Geometry::Point<T> p2, Geometry::Point<T> p3,
...
template <typename T>
double Geometry::signed_area_of_parallelogram(Geometry::Point<T> a, Geometry::Point<T> b, Geometry::Point<T> c)
{
    return (cross(b - a, c - b).z);
}
template <typename T>
double Geometry::area_of_triangle(Geometry::Point<T> a, Geometry::Point<T> b, Geometry::Point<T> c)
{
    return abs(Geometry::signed_area_of_parallelogram(a, b, c) * 0.5);
}
template <typename T>
int Geometry::direction(Geometry::Point<T> a, Geometry::Point<T> b, Geometry::Point<T> c)
{
...
double Geometry::area_of_polygon(vector<Geometry::Point<T>> a, int n)
{
    ...
    static_assert(
        n >= 0 && n <= a.size(), "size must be less than the array size ");
    if (n == 0)
        n = a.size();
    double area = 0;
    for (int i = 0; i < n; i++)
...
bool Geometry::check_Point_in_polygon(Geometry::Point<T> a[], int n, Geometry::Point<T> p)
{
    // query time O(log n)
    int l = 1, r = n - 2;
    while (l < r)
...
template <typename T>
std::vector<Geometry::Point<T>> minkowski_sum(std::vector<Geometry::Point<T>> a, std::vector<Geometry::Point<T>> b,
...
std::vector<Geometry::Point<T>> remove_redundant(std::vector<Geometry::Point<T>> v, int n)
{
    static_assert(!v.empty(), "array cannot be empty");
    static_assert(n >= 0 && n <= v.size(), "n must be greater than 0 and less than the size of the array");
...
template Geometry::Point<double> Geometry::cross(Geometry::Point<double> a, Geometry::Point<double> b);