...
namespace Geometry
{
    ...
    template <typename T>
    float ang(Point<T> a, Point<T> b, bool deg)
    {
    ...
    Point<T> cross(Point<T> a, Point<T> b)
    {
        ...
        return Point<T>(y0 * z1 - z0 * y1 + 0.00, z0 * x1 - x0 * z1 + 0.00,
                        x0 * y1 - y0 * x1 + 0.00);
    }
    template <typename T>
    ...
    template <typename T>
    double area_of_polygon(vector<Point<T>> a, int n)
    {
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                else
                {
                    ...
                    if (d > 2 * r)
                        continue;
                    double ang1 =
                        atan(((double)(v[j] - v[i]).y) / (double)(v[j] - v[i]).x);
                    if ((v[j] - v[i]).x < 0)
                        ang1 += M_PI;
    ...
    vector<Point<T>> convex_hull_jarvis_march(vector<Point<T>> a)
    {
        ...
        {
            if (a[i].x < a[start].x ||
                (a[i].x == a[start].x && a[i].y < a[start].y))
                start = i;
        }
        vector<Point<T>> ans;
        ans.pb(a[start]);
    ...
    vector<Point<T>> convex_hull_graham_scan(vector<Point<T>> a)
    {
        ...
        for (int i = 2; i < n; i++)
        {
            while (direction(ans[(int)ans.size() - 2], ans[(int)ans.size() - 1],
                             a[i]) != 1)
                ans.pop_back();
            ans.push_back(a[i]);
        }
